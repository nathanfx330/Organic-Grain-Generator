...
class OrganicGrainGeneratorApp:
    def __init__(self, master):
        ...
        master_control_frame = ttk.Frame(self.master, width=350, style="Dark.TFrame")
    ...
    def setup_dark_theme(self):
        ...
        BUTTON_COLOR = "#4A4A4A"
        TROUGH_COLOR = "#333333"
        HANDLE_COLOR = "#CDCDCD" # 80% Grey
        
        style = ttk.Style(self.master)
        
        # This is the key: Use a theme that can be reliably colored. 'clam' is the best.
        # The previous mistake was not styling it completely, leading to an ugly result.
        # This version styles it to be dark and professional.
        style.theme_use('clam')

        # --- Global & Base Widget Styling ---
        style.configure(".",
                        background=BG_COLOR,
                        foreground=FG_COLOR,
                        fieldbackground=SELECT_BG,
                        borderwidth=0,
                        lightcolor=BG_COLOR, # Removes 3D border effects
                        darkcolor=BG_COLOR) # Removes 3D border effects
        ...
        # Entry and Combobox (removes ugly default focus ring)
        ...
        # --- CORRECTED SLIDER STYLE ---
        # This style keeps the native feel while enforcing dark colors.
        style.configure("Horizontal.TScale",
                        troughcolor=TROUGH_COLOR,
                        background=HANDLE_COLOR, # The handle itself
                        gripcount=0)
        style.map("Horizontal.TScale",
                  background=[('active', 'white'), ('pressed', 'white')])

        # Force the root window and non-ttk canvas widgets to have the correct background
    ...
    def _on_mouse_wheel(self, event):
        if event.num == 5 or event.delta == -120: self.control_canvas.yview_scroll(1, "units")
        elif event.num == 4 or event.delta == 120: self.control_canvas.yview_scroll(-1, "units")
    ...
    def _on_canvas_scroll_zoom(self, event):
        ...
        if event.num == 5 or event.delta < 0: self.current_zoom_index = max(0, self.current_zoom_index - 1)
        elif event.num == 4 or event.delta > 0: self.current_zoom_index = min(len(self.zoom_levels) - 1, self.current_zoom_index + 1)
        else: return
    ...
    def create_widgets(self):
        # ... (All widget creation code is identical to previous version)
        ...
        self.sliders = {}
        slider_params = {"PRNU (Gain FPN)": (0, 5.0, 0), "DSNU (Offset FPN)": (0, 10.0, 0),"Shot Noise (Poisson)": (0, 5.0, 0), "Read Noise (Gaussian)": (0, 15.0, 0),"Color Noise": (0, 20.0, 0),"Shadow Noise Bias": (0, 5.0, 0),"Banding": (0, 0.1, 0), "Bit Depth": (4, 8, 8),"Firefly Density (%)": (0, 1.0, 0), "Firefly Intensity": (0, 500.0, 0),"Firefly Coloration": (0, 2.0, 0)}
    ...
    def _generate_noise_array(self, seed_offset=0, composite=True):
        ...
        else: final_array = grain_plate_arr
        return final_array
    ...
    def _generate_grain_plate(self, width, height, seed_offset, luma_mask=None):
        ...
        if luma_mask is not None:
            shot_noise_intensity *= luma_mask; read_noise_strength *= luma_mask; color_intensity *= luma_mask
        if np.any(shot_noise_intensity > 0):
            photon_scale = 50.0
            ...
            luma_image = luma_image + shot_noise * (shot_noise_intensity / np.maximum(1.0, shot_noise_intensity))
        ...
        if np.any(color_intensity > 0):
            color_noise_map = rng.normal(0.0, 1.0, final_image.shape).astype(np.float32)
            final_image += color_noise_map * color_intensity[..., np.newaxis]
    ...
    def _get_fixed_maps_for_resolution(self, width, height):
        ...
        perlin = PerlinNoise(octaves=6, seed=self.get_master_seed())
    ...
    def _update_display_image(self):
        ...
        zoom_level_str = self.zoom_var.get(); img_w, img_h = self.pil_image.size
        if zoom_level_str == "Fit to Window":
            frame_w = self.canvas.winfo_width() - 4; frame_h = self.canvas.winfo_height() - 4
    ...
    def on_slider_update(self, event=None):
        if self.initializing: return
        if self.realtime_preview_var.get(): self.update_noise()
        elif self.zoom_window: self.update_noise()
    ...
    def load_background_image(self):
        filepath = filedialog.askopenfilename(title="Select Background Image", filetypes=[("Image Files", "*.png *.jpg *.jpeg"), ("All files", "*.*")])
    ...
    def export_sequence(self):
        try:
            ...
            if start_frame > end_frame or not prefix: return
        ...
        self.progress_bar["maximum"] = end_frame - start_frame + 1; self.progress_bar["value"] = 0
    ...
    def _export_worker(self, filepath, start_frame, end_frame, prefix):
        ...
        for i, frame_num in enumerate(range(start_frame, end_frame + 1)):
            ...
            Image.fromarray(final_array).save(os.path.join(filepath, f"{prefix}.{frame_num:04d}.png"))
    ...
    def _export_done_ui_cleanup(self): self.export_button.config(state="normal"); self.master.config(cursor=""); self.progress_bar["value"] = 0

    def save_image(self):
        if not self.pil_image: return
        filepath = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png"), ("JPEG files", "*.jpg"), ("All files", "*.*")])
        if not filepath: return
        do_composite = self.export_mode_var.get() == "Composited Image" and self.background_pil_image
        if do_composite: self.pil_image.save(filepath)
        else: Image.fromarray(self._generate_noise_array(seed_offset=0, composite=False)).save(filepath)
